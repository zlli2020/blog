...
Pareto and control charts are used together to help tunnel down into the operation process and to find the root cause of productivity problems. The Pareto chart helps to identify and focus effort on the top portion of the causes in order to resolve the majority of the problems. Using a variety type of control charts with control limits of 3 standard deviations, we can study the stability of the current process, analyze and make improvement, and monitor the result when the new process is implemented. Besides, the data obtained from the process can also be applied to predict future performances of the process. 

The data are examined using a control chart to determine if the problem is special causes of variation or just common causes of variation. ([source](https://cran.r-project.org/web/packages/qicharts2/vignettes/qicharts2.html#understanding-variation )). 
...
Data filter: To allow users to filter and zoom into the data by Date, Terminal, Equipment and container types to examine the patterns across time and space. This also allows users to flow from high-level overview and narrow down to data on a higher granularity.

Use of tooltips: To enhance user experience when user hovers over the chart objects to provide on-demand details without having too much information cluttered on the chart.

When we explore and try to make sense of data, the Data visualization help us to follow the
flow of "overview first, zoom and filter into details", bouncing back and forth,
...
qcc is easy to use and provides comprehensive plot of control chart with a simple and a more traditional statistical reporting plot. However, it does not support ggplotly for interactivity and the data need to be grouped into wide table with each observation as a separate column.
...
qichart2 provides comparable easy to use function to generate the control charts. It is also aesthetically clean and pleasing and qic() can be integrated into ggplot and plotly for interactivity. (Details will be discussed in the next segment.)
...
We use Xbar and S control charts together for the average measurement of PM Wait Time
...
Facets is used to provide an overview breakdown by the various attributes of the measurement. In which filters can be applied to focus on the those that shows higher fluctuation outside the control limits.
...
### 2.4.2 Individual(I) and Moving Range(MR) chart

I and MR Control charts are used together to show absolute difference between each data points. which allows us to looking into individual events that are out of the control limits. From the I-chart  we can see that operations no 29 and 77 are out of the upper control limit(UCL), to investigate further for the root cause. While for MR-chart shows the extreme change in waiting time point 29 and 30. 
...
### 2.4.2 Count(C), Rate(U) & Proportion(U) Control Chart

We use C-Chart to count the number of events that exhibits high PM Waiting time. This in turn will allow us to spot any spike changes of high wait time events.
...
While the Proportion P-Chart is used to plot the proportion of PM high wait time. 
...
Lastly the Rate U-Chart look into the rate by the number of Long PM wait time events per 1000 containers/events. Different from P-Chart, the denominator can be changed to other measurement such as per 10 KM traveled or Working hours.
...
# 3.0 Sketch of Proposed Visualisation
