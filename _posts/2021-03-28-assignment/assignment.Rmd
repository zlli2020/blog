...
Data filter: To allow users to filter and zoom into the data by Date, Terminal, Equipment and container types to examine the patterns across time and space. 

Use of tooltips: To provide details when user hovers on the chart objects. 
...
In this assignment, we will be using PM wait time as the measurement which can be toggled across the different time indicators. 
...
Although the ggplot Pareto chart can be rendered into plotly to have interactivity such as mouse hover tooltip for details, it does not work well with second Y axis. Thus, the static ggplot is more useful to provide the necessary information required.
...
We use C-Chart to count the number of events that shows high PM Waiting time. This in turn will allow us to spot any spike changes of high wait time events.
...
Lastly the Rate U-Chart look into the rate by the number of Long PM wait time events per 1000 containers/events.
