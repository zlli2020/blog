...
Pareto and control charts are used together to help tunnel down into the operation process and to find the root cause of productivity problems. The Pareto chart helps to identify and focus effort on the top portion of the causes in order to resolve the majority of the problems. Using a variety type of control charts with control limits of 3 standard deviations, we can study the stability of the current process, analyze and make improvements, and monitor the result when the new process is implemented. Besides, the data obtained from the process can also be applied to predict future performances of the process. 
...
Use of tooltips: To enhance the user experience when the user hovers over the chart objects to provide on-demand details without having too much information cluttered on the chart.

When we explore and try to make sense of data, the Data visualization helps us to follow the
the flow of "overview first, zoom and filter into details", bouncing back and forth,
...
The data source used in this exercise is from PSA Singapore’s PM anonymized dataset that contains PM operation event records. There are approximately 5 million records with 69 variables. The data set is reduced to 37 variables for the preliminary analysis. 
...
The studies focus on PM productivity in the yard, we filtered to take in only yard event_c which are Equipment Mount(EQMT) and Equipment Offload(EQOF) operations. Unused columns are removed using subset() function to further reduced the data set. 
...
Use of annotation to indicate Upper Control Limit(UCL), Lower Control Limit(LCL), and Central Line (CL). (The code are repeating for the other control charts)
```{r}
...
It is important to note that the control chart can identify non-random variation, it cannot identify its root cause. The analysis can only tell that PM waiting time has been reduced or stabilized after an implemented change. However, it cannot be established as the cause of the change. 

While more data points may increase the sensitivity of the chart, more observation will increase the risk of more false positives, reducing the specificity of the chart. Thus, determining the range and scope of the analysis is critical.

The PM waiting time and traveling time are estimated base on distance traveled minus non-work time and unproductive time. The accuracy can be improved if the data set captures the actual traveling time and waiting time using GPS or IoT sensor data. In which, future Tuas terminals Automated Guided Vehicles (AGV) would replace the majority of the current traditional human-driven Prime Movers to be the new norm. Hence using the similar approach, we can use Pareto and Control charts to analyze the stability of current and future processes.
